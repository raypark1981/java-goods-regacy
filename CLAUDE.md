...
## 프로젝트 성격 (중요)
- 이 프로젝트는 **실제로 구동/배포하지 않는다.** 회사 내부망 프로젝트의 구조와 소스 패턴을 참조·학습하기 위해 껍데기만 유사하게 재현한 것.
- 따라서 미사용 함수/속성, dead code, mock 데이터, DB 컬럼 세팅 누락, 컴파일·런타임 동작 여부 등은 **지적하거나 고치려 하지 말고 무시한다.**
- 코드 리뷰·검토 요청 시에도 "동작하는지"가 아니라 **네이밍/패키지 구조/설계 패턴이 개발표준과 문서 흐름에 맞는지**에 초점을 맞춘다.
- 사용자가 명시적으로 특정 파일/함수를 고쳐달라고 요청한 경우에만 수정한다.
