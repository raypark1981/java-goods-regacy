...
public interface PEAAutoChargeService {
    ...
    PEAAutoChargeResVO issueBillingKey(PEAAutoChargeReq.BillingAuthIssue reqVO, HttpServletRequest request) throws Exception;

    /** 자동충전 결제 요청. 빌링키 조회 → 금액 산정 → 토스 결제 → NICE 포인트 충전 순으로 처리한다. */
    PEAAutoChargeResVO requestAutoCharge(PEAAutoChargeReq.Charge reqVO) throws Exception;

    /** 자동충전 결제 취소 요청. NICE 포인트 충전 실패 시 토스 결제를 취소한다. */
    PEAAutoChargeResVO cancelAutoCharge(PEAAutoChargeReq.Cancel reqVO) throws Exception;
}
